...
func ReceptionistController(c *gin.Context) {
	c.JSON(http.StatusOK, gin.H{"message": "Welcome to Receptionist Dashboard"})
}
...
func CreatePatientRecord(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"message": "Successfully Created", "patient": record})
}
...
func DeletePatientRecord(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"message": fmt.Sprintf("Patient with ID %s deleted", patientID)})
}
